...
def get_visit_ta_image(visitid,
    ...
    if verbose:
        print(f"Querying MAST for target acquisition files (visit {visitid})")
    ...
    if verbose:
        print(f"Found {nfiles} target acquisition image(s).")
...
def show_ta_img(visitid,
    ...
    if inst == 'MIRI':
        spaceKLIP.target_acq_tools._crop_display_for_miri(ax, hdul)
...
def ta_analysis(data_product,
    ...
    targ_coords_pix_adj_sci : tuple
        The adjusted science target coordinate in pixels (in the sci aperture).
    """

    # Check inputs.
    # Standardize format to 'VPPPPPOOOVVV'.
    visitid = utils.get_visitid('V'+fits.getval(data_product, 'VISIT_ID'))
    ...
    n_ta_imgs = (1 if isinstance(ta_imgs, astropy.io.fits.HDUList)
                 else len(ta_imgs))
    ...
    stored_wcs_offsets = None
    for i_ta_image in range(n_ta_imgs):
        if plot:
            (hdul, ax, norm, cmap,
             bglevel, inset_ax) = show_ta_img(visitid, ax=axes[i_ta_image],
                                              return_handles=True, inst=inst,
                                              ta_expnum=i_ta_image + 1,
                                              zoom=zoom,
                                              verbose=False,
                                              **kwargs)
        else:
            if isinstance(ta_imgs, list) and not isinstance(ta_imgs,
                                                            fits.HDUList):
                hdul = ta_imgs[i_ta_image]

        # Determine reference point (intended target position).
        xref, yref = get_ta_reference_point(inst, hdul, i_ta_image + 1)
        aperture_text = (f"Intended target pos (from SIAF): "
                         f"{xref:.2f}, {yref:.2f}")
        ...
        if inst in ['NIRCAM', 'MIRI']:
            full_ap = (
                siaf[ta_aperture.AperName[:5] + "_FULL"]
                if inst == "NIRCAM"
                else siaf["MIRIM_FULL"]
            )
        ...
        try:
            ...
            try:
                ...
                # Convert from 1-based to 0-based indexing for all centroids.

                # Centroid in the subarray coordinate frame.
                oss_cen_sci_pythonic = np.asarray(oss_cen_sci) - 1
                # Centroid in the full-frame coordinate frame.
            ...
            # Compute positional offset (Δpos) if OSS centroid is available.
            if not np.isnan(oss_cen_sci_pythonic[0]):
                deltapos = (oss_cen_sci_pythonic[0] - xref,
                            oss_cen_sci_pythonic[1] - yref)
                deltapos_type = 'OSS - Intended'

            if plot:
                ax = axes[i_ta_image]  # Select correct subplot for TA image.
                oss_centroid_text = (f"OSS centroid: "
                                     f"{oss_cen_sci_pythonic[0]:.2f}, "
                                     f"{oss_cen_sci_pythonic[1]:.2f}" or "")

                ax.scatter(*oss_cen_sci_pythonic, color='r',
                           marker='x', s=50)
                ax.text(*oss_cen_sci_pythonic, 'OSS  ', color='r',
                        verticalalignment='center',
                        horizontalalignment='right')
                inset_ax.scatter(*oss_cen_sci_pythonic, color='r',
                                 marker='x', s=50)
                inset_ax.text(*oss_cen_sci_pythonic, 'OSS  ', color='r',
                              verticalalignment='center',
                              horizontalalignment='right')
            ...
            # Convert celestial coordinates (RA, Dec)
            # to pixel coordinates using WCS.
            ...
            if plot:
                ...
                inset_ax.scatter(targ_coords_pix[0], targ_coords_pix[1],
                                 color='magenta', marker='+', s=50)
                inset_ax.text(targ_coords_pix[0], targ_coords_pix[1] + 2,
                              'WCS', color='magenta',
        ...
        if np.isnan(oss_cen_sci_pythonic[0]):
        ...
        image_text = (f"Pixel coordinates (0-based):\n{oss_centroid_text}\n"
                      ...
                      f"{wcs_text}\n{aperture_text}\n"
                      f"$\\Delta$pos ({deltapos_type}): {deltapos[0]:.2f}, "
                      f"{deltapos[1]:.2f}")
        if plot:
            ax.text(0.95, 0.04, image_text, horizontalalignment='right',
                    verticalalignment='bottom',
                    transform=ax.transAxes, color='k',
                    bbox=dict(facecolor='0.75', alpha=0.9, edgecolor='none',
                              boxstyle='round,pad=0.3'))
        ...
        if oss_cen_sci_pythonic is not None:
            ...
            stored_wcs_offsets = original_wcs_offset_radec

            wcs_offset_pix = (np.asarray(targ_coords_pix)
                              - oss_cen_sci_pythonic)  # Offset pixels (x,y).
            ...
            ta_cen_coords_adj = wcs_ta_adj.pixel_to_world(
            ...
            targ_coords_pix_adj_sci = wcs_sci_adj.world_to_pixel(targ_coords)

            # Compute any residuals after adjustment.
            residual_pix = (np.asarray(targ_coords_pix_adj)
                            - oss_cen_sci_pythonic)
            dra_residual = (targ_coords_adj.ra - ta_cen_coords_adj.ra)
            ddec_residual = (targ_coords_adj.dec - ta_cen_coords_adj.dec)
            residual_radec = (dra_residual.to(u.arcsec),
                              ddec_residual.to(u.arcsec))

            if plot:
                ax.scatter(targ_coords_pix_adj[0], targ_coords_pix_adj[1],
                           color='k', marker='+', s=50)
                ax.text(targ_coords_pix_adj[0], targ_coords_pix_adj[1] + 2,
                        'WCS_ADJ', color='k',
                        verticalalignment='bottom',
                        horizontalalignment='center')
                inset_ax.scatter(targ_coords_pix_adj[0],
                                 targ_coords_pix_adj[1],
                                 color='k', marker='+', s=50)
                inset_ax.text(targ_coords_pix_adj[0],
                              targ_coords_pix_adj[1] + 2,
                              'WCS_ADJ', color='k',
                              verticalalignment='center',
                              horizontalalignment='left')
                ...
                annotations = [
                    (f"WCS $\\Delta$RA, $\\Delta$Dec = "
                     ...
                     f"{ddec.to_value(u.arcsec):.3f} arcsec", 'magenta', 0.80),
                    (f"WCS_ADJ $\\Delta$RA, $\\Delta$Dec = "
                     f"{dra_residual.to_value(u.arcsec):.3f}, "
                     f"{ddec_residual.to_value(u.arcsec):.3f} arcsec",
                     'k', 0.75)
                ]
            ...
            if verbose:
                ...
                print(f"TA Target coordinates (RA, Dec) [deg]: "
                      f"({targ_coords.ra.deg:.6f}, {targ_coords.dec.deg:.6f})")
                print(f"TA Target (hms | dms): "
                      f"{targ_coords.to_string('hmsdms', sep=':')}")
                print(f"TA Target coordinates from WCS (x, y) [pix]: "
                      f"{targ_coords_pix[0]:.3f}, {targ_coords_pix[1]:.3f}\n")

                print("\n------TA OSS Centroid Information------")
                print(f"TA Target centroid coordinates (RA, Dec) [deg]: "
                      f"({ta_cen_coords.ra.deg:.6f}, "
                      f"{ta_cen_coords.dec.deg:.6f})")
                print(f"OSS centroid on board (x, y):  {oss_cen}  "
                      f"(full det coord frame, 1-based)")
                print(f"OSS centroid converted (x, y): {oss_cen_sci_pythonic} "
                      f"(sci frame in {ta_aperture.AperName}, 0-based)")
                print(f"OSS centroid converted (x, y): {oss_cen_full_sci} "
                      f"(sci frame in {full_ap.AperName}, 0-based)\n")

                print("\n------Offset Information------")
                print(f"TA WCS offset (x, y) [pix] = {wcs_offset_pix[0]:.3f}, "
                      f"{wcs_offset_pix[1]:.3f} (WCS - OSS)")
                print(f"TA ΔRA, ΔDEC = {original_wcs_offset_radec[0]:.3f}, "
                      f"{original_wcs_offset_radec[1]:.3f} (WCS - OSS)\n")

                print("\n------Adjusted Target Coordinates------")
                print(f"Adjusted TA Target coordinates (RA, Dec) [deg]: = "
                      f"{targ_coords_adj.ra.deg:.6f}, "
                      f"{targ_coords_adj.dec.deg:.6f}")
                print(f"TA Target coordinates from Adjusted WCS (x,y) [pix]: "
                      f"= {targ_coords_pix_adj[0]:.3f}, "
                      f"{targ_coords_pix_adj[1]:3f}")

                print(f"Residual offset (x, y) [pix] = {residual_pix[0]:.3f}, "
                      f"{residual_pix[1]:.3f}")
                print(f"Residual offset (RA, DEC) [arcsec] = "
                      f"{residual_radec[0]:.6f}, {residual_radec[1]:.6f}\n")

        if stored_wcs_offsets:

            wcs_ta_adj = adjust_wcs(wcs_ta, *stored_wcs_offsets)
            targ_coords_pix_adj = wcs_ta_adj.world_to_pixel(targ_coords)
            targ_coords_adj = wcs_ta_adj.pixel_to_world(*targ_coords_pix_adj)
    ...
    if plot:
        ...
        outname = os.path.join(output_dir,
                               f'{inst.lower()}_ta_analysis_{visitid}.pdf')
        plt.savefig(outname)
        print(f" => {outname}")

    if oss_cen_sci_pythonic is not None:
        print(targ_coords_pix_adj_sci)
