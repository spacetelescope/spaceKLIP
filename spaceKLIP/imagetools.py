...
class ImageTools():
    ...
    def crop_frames(self,
        ...
        for i, key in enumerate(self.database.obs.keys()):
            ...
            for j in range(nfitsfiles):
                ...
                # Update spaceKLIP database.
                self.database.update_obs(key, j, fitsfile, maskfile, nanmaskfile=nanmaskfile, crpix1=crpix1, crpix2=crpix2)
    ...
    def blur_frames(self,
        ...
        for i, key in enumerate(self.database.obs.keys()):
            ...
            for j in range(Nfitsfiles):
                ...
                # Update spaceKLIP database.
                if fact_temp is None:
                    self.database.update_obs(key, j, fitsfile, maskfile,  nanmaskfile=nanmaskfile, blurfwhm=np.nan)
                else:
                    self.database.update_obs(key, j, fitsfile, maskfile,  nanmaskfile=nanmaskfile, blurfwhm=fact_temp * np.sqrt(8. * np.log(2.)))
        ...
        for i, key in enumerate(self.database.obs.keys()):
            ...
            if recenter_NICAM_with_MCMC:
                ...
                ax.axhline(0., color='gray', lw=1,
                           zorder=-1)  # set zorder to ensure lines are drawn behind all the scatter points
                ...
                ax.set_title(
                    f'Science frame alignment\nfor {self.database.obs[key]["TARGPROP"][ww_sci[0]]}, {self.database.obs[key]["FILTER"][ww_sci[0]]}')
                if save_figures:
                    output_file = os.path.join(output_dir, key + '_MCMC_aligned_sci.pdf')
                    plt.savefig(output_file)
                ...
                if len(ww_ref) > 0:
                    ...
                    ax.set_title(
                        f'Reference frame alignment\n showing {len(ww_ref)} PSF refs for {self.database.obs[key]["FILTER"][ww_ref[0]]}')
                    if save_figures:
                        output_file = os.path.join(output_dir, key + '_MCMC_aligned_ref.pdf')
                        plt.savefig(output_file)
                        log.info(f" Plot saved in {output_file}")
                    plt.show()
                    plt.close(fig)
    ...
    def align_frame_processor(self, j, l, key, fitsfile_ref, output_dir,
                              method='fourier',
                              align_algo='leastsq',
                              mask_override=None,
                              msk_shp=8,
        ...
        """
            ...
            j : int
                index in the ww_all.
            l : int
                first index in the ww_sci.
            key: str.
                A key in the database.obs.keys()
            fitsfile_ref: str
                path to the fitsfile to use as reference for the allingemnt.
            output_dir: str
    ...
    def align_frames(self,
                     ...
                     save_figures=True):
        """
        Wrapper to prarallelize (or not) the alligne_frames_prosessor() to align all SCI and REF frames to the first SCI frame.
        ...
        parallelize: bool, optional
            If True, enable parallel processing. Default is False.
        ...
        for i, key in enumerate(self.database.obs.keys()):
            ...
            if parallelize:
                with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
                    executor.map(self.align_frame_processor, ww_all,  repeat(ww_sci[0]), repeat(key),
                                repeat(fitsfile_ref), repeat(output_dir), repeat(method),
                                repeat(align_algo), repeat(mask_override), repeat(msk_shp), repeat(shft_exp),
                                repeat(align_to_file), repeat(scale_prior), repeat(threshold),
                                repeat(kwargs), repeat(remove_frames), repeat(types),
                                repeat(remove_frames_subdir), repeat(False), repeat(subdir))
            else:
                for j in ww_all:
                    self.align_frame_processor(j, ww_sci[0], key, fitsfile_ref, output_dir, method, align_algo,
                                               mask_override, msk_shp, shft_exp, align_to_file, scale_prior,
                                               threshold,kwargs, remove_frames, types, remove_frames_subdir,
                                               verbose, subdir)
